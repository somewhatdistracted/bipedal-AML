	...
	for j in range(202):
		if j > 0:
			for element in final_csv[(j-1) + 202*i]:
				direction_csv[j + 202*i].append(float(element))
		else:
...
for row in direction_csv:
	...
	for element in row:
		new_row.append(float(element))
	direction_5_csv.append(new_row)

for i in range(24):
	for j in range(202):
		if j > 4:
			for element in final_csv[(j-5) + 202*i]:
				direction_5_csv[j + 202*i].append(float(element))
		else:
			for element in final_csv[(202-(5-j)) + 202*i]:
				direction_5_csv[j + 202*i].append(float(element))
		
		print(j + 202*i)	
			
with open('newSubjectDataDirection5.csv', mode='w') as new_file:
		file_writer = csv.writer(new_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
		
		for row in direction_5_csv :
